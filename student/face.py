...
def scan_known_people_for_known_names(known_people_folder):
    ...
    for file in image_files_in_folder(known_people_folder):
        ...
        if os.path.exists("./media/encodings/" + basename + ".arr.npy"):
            encodings = np.array([np.load("./media/encodings/" + basename + ".arr.npy")])
        else:

            encodings = face_recognition.face_encodings(img, None, 4)
            np.save("./media/encodings/" + basename + ".arr", encodings[0])
        if len(encodings) > 1:
...
def scanClass(known_people_folder, image_to_check, class_crn):
    ...
    imagefile = open("./media/classPhoto/" + class_crn + ".jpg", "rb")
...
@click.option('--image_to_check', default="./media/classPhoto/")
@click.option('--known_people_folder', default="./media/studentPhotos/")
